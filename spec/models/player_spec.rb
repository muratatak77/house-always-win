...
RSpec.describe Player, type: :model do
  ...
  describe "dependent behavior" do
    it 'destroys game_sessions when player is destroyed' do
      ...
      open_session = create(:game_session, player: player, status: :open)
      closed_session = create(:game_session, player: player, status: :closed)
    ...
    it "nullifies cash_outs.player_id when player is destroyed" do
      ...
      expect { owner.destroy }.to change { Player.where(id: owner.id).count }.from(1).to(0)
